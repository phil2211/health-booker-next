...
export default function BookingConfirmation({
  ...
  const generateICSCalendarEvent = () => {
    ...
    const cancellationUrl = cancellationToken
      ...
      : t('calendar.contactUsToCancel')

    const description = `${t('calendar.appointmentDetails')}:
• ${t('calendar.therapist')}: ${therapistName}
• ${t('calendar.treatment')}: ${t('booking.cranioSacralSession')}
• ${t('calendar.patient')}: ${patientName}
• ${t('calendar.email')}: ${patientEmail}
• ${t('calendar.duration')}: ${t('booking.oneHour')}

${t('calendar.toCancelVisit')}: ${cancellationUrl}

${t('calendar.bookedThrough')}`
...
SUMMARY:${t('booking.cranioSacralSession')} - ${therapistName}
DESCRIPTION:${description.replace(/\n/g, '\\n')}
LOCATION:${t('calendar.virtualRemoteSession')}
STATUS:CONFIRMED
ORGANIZER;CN=${therapistName}:mailto:appointments@healthbooker.app
ATTENDEE;CN=${patientName}:mailto:${patientEmail}
BEGIN:VALARM
ACTION:DISPLAY
DESCRIPTION:${t('calendar.reminderTomorrow', { therapistName })}
TRIGGER:-P1D
END:VALARM
BEGIN:VALARM
ACTION:DISPLAY
DESCRIPTION:${t('calendar.reminderOneHour', { therapistName })}
TRIGGER:-PT1H
END:VALARM
END:VEVENT
END:VCALENDAR`
  ...
  const addToAndroidCalendar = () => {
    ...
    const title = encodeURIComponent(t('calendar.cranioSacralSessionWith', { therapistName }))
    const description = encodeURIComponent(`${t('calendar.appointmentWith', { therapistName })}\n${t('calendar.patient')}: ${patientName}\n${t('calendar.email')}: ${patientEmail}`)
  ...
  const formatAppointmentDateTime = () => {
    ...
    const timeStr = new Date(`${appointmentDate}T${startTime}:00`).toLocaleTimeString(locale === 'de' ? 'de-DE' : 'en-US', {
      hour: 'numeric',
      minute: '2-digit',
      hour12: true,
    })
    ...
    <div className="bg-green-50 border border-green-200 rounded-xl shadow-lg p-2 sm:p-3 lg:p-4">
      <div className="flex items-start">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg sm:text-xl font-bold text-green-900 mb-1 sm:mb-2">{t('booking.bookingConfirmed')}</h3>
          <p className="text-green-800 mb-3 sm:mb-4 text-sm">
            {t('booking.appointmentBooked')} <strong>{patientEmail}</strong>.
          </p>

          {/* Appointment Details */}
          <div className="bg-white rounded-lg border p-2 sm:p-3 mb-3 sm:mb-4">
            <h4 className="text-sm sm:text-base font-semibold text-gray-900 mb-2 sm:mb-3">{t('booking.appointmentDetails')}</h4>
          ...
          <div className="bg-white rounded-lg border p-2 sm:p-3 mb-3 sm:mb-4">
            <h4 className="text-sm sm:text-base font-semibold text-gray-900 mb-2 sm:mb-3">{t('booking.addToCalendar')}</h4>
            <p className="text-gray-600 mb-2 sm:mb-3 text-xs sm:text-sm">
              {t('booking.calendarInstructions')}
            </p>
              ...
              <div className="space-y-1.5 sm:space-y-2">
                ...
                {deviceType === 'android' && (
                  ...
                  >
                    ...
                    {t('booking.addToAndroidCalendar')}
                  </button>
                )}
                ...
                {deviceType === 'desktop' && (
                  ...
                  >
                    ...
                    {navigator.platform.toLowerCase().includes('mac')
                      ? t('booking.openInCalendarApp')
                      : t('booking.downloadCalendarFile')}
                  </button>
                )}

                <div className="text-center text-xs text-gray-500 mt-1.5 sm:mt-2">
                  {deviceType === 'ios' && t('booking.iosCalendarHint')}
                  {deviceType === 'android' && t('booking.androidCalendarHint')}
                  {deviceType === 'desktop' && (
                    navigator.platform.toLowerCase().includes('mac')
                      ? t('booking.macCalendarHint')
                      : t('booking.desktopCalendarHint')
                  )}
                </div>
              </div>
            )}
