...
export default function PatientBookingScheduler({ therapistId, blockedSlots = [], onBookingConfirmed, therapistName }: PatientBookingSchedulerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedSlot || !selectedDate) {
      setSubmitError(t('pleaseSelectDateAndTime'))
      return
    }

    if (!patientName.trim() || !patientEmail.trim()) {
      setSubmitError(t('pleaseFillRequiredFields'))
      return
    }
    ...
    if (!emailRegex.test(patientEmail)) {
      setSubmitError(t('pleaseEnterValidEmail'))
      return
    }
    ...
    try {
      ...
      setSubmitError(err instanceof Error ? err.message : t('errorCreatingBooking'))
    } finally {
      setSubmitting(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      {!submitSuccess && (
        <div className="bg-indigo-50 border border-indigo-200 rounded-lg p-4">
          <p className="text-sm text-indigo-800 font-medium">{t('treatment')}</p>
          <p className="text-lg text-indigo-900 font-semibold">{t('treatmentType')}</p>
        </div>
      )}
      ...
      {!submitSuccess && (
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-gray-500">{t('timesInLocalTimezone')}</p>
        </div>
      )}
      ...
      {!submitSuccess && (
        <div className="space-y-2">
          <label className="block text-sm font-medium text-gray-700">{t('selectTime')}</label>
          {loading && (
            <div className="text-center py-8 text-gray-500">{t('loadingAvailableTimes')}</div>
          )}
          {error && (
              ...
              {slots.map((slot) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="text-xs text-gray-500 mt-1">{t('oneHourSession')}</div>
                  </button>
                )
              })}
            </div>
      ...
      {selectedSlot && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-4">
          <p className="text-sm text-green-800 font-medium">{t('selectedAppointment')}</p>
      ...
      {selectedSlot && !submitSuccess && (
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('patientInformation')}</h3>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label htmlFor="patient-phone" className="block text-sm font-medium text-gray-700 mb-1">
                  {t('phoneNumber')} <span className="text-gray-500 text-xs">{t('phoneOptional')}</span>
                </label>
                <input
                  id="patient-phone"
                  type="tel"
              ...
              <div>
                <label htmlFor="patient-comment" className="block text-sm font-medium text-gray-700 mb-1">
                  {t('specialRequirements')} <span className="text-gray-500 text-xs">{t('phoneOptional')}</span>
                </label>
                <textarea
                  id="patient-comment"
